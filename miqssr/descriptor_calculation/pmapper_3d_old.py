...
from .pmapper_old import pharmacophore as P
from .read_input import read_input

smarts = P.read_smarts_feature_file(pkg_resources.resource_filename(__name__, 'pmapper_old/smarts_features.txt'))
...
# col_clean can use for test set
    ...
    if col_clean is None:
        ...
        if len(col_clean) == 0:
            ...
            col_clean = get_phf_clean(fname=fname, ncpu=ncpu, part=0.0)
    ...
    phf = pd.DataFrame(phf_res[1], columns=col_clean).merge(pd.DataFrame(phf_res[0], columns=['mol_title', 'act', 'mol_id']), left_index=True, right_index=True, copy=False)
